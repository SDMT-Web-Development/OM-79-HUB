...
namespace OM_79_HUB.Controllers
{
    public class HomeController : Controller
    {
        ...
        "DistrictAdmin"
            ...
            // Organize central office roles
            var centralOfficeRoles = new Dictionary<string, List<string>>
    {
        { "Regional Engineer", new List<string>() },
        { "HDS", new List<string>() },
        { "GIS Manager", new List<string>() },
        { "Chief", new List<string>() },
        { "Director of Operations", new List<string>() }
    };
            ...
            foreach (var user in users)
            {
                if (user.RegionalEngineer)
                {
                    ...
                    centralOfficeRoles["Regional Engineer"].Add($"{user.FirstName} {user.LastName}|{user.Email}|{districts}");
                }
                if (user.HDS)
                {
                ...
                if (user.GISManager)
                {
                    centralOfficeRoles["GIS Manager"].Add($"{user.FirstName} {user.LastName}|{user.Email}");
                }
                if (user.Chief)
                {
                ...
                {
                    centralOfficeRoles["Director of Operations"].Add($"{user.FirstName} {user.LastName}|{user.Email}");
                }
            }
